...
public class DepthOfFieldAutoFocus : MonoBehaviour, IDepthOfFieldAutoFocus
{
    ...
    bool Supported()
    {
        ...
        // Check for "UAVs at any pipeline stage" support.
#if UNITY_EDITOR_WIN || UNITY_STANDALONE_WIN || UNITY_EDITOR_LINUX || UNITY_STANDALONE_LINUX
        // For windows, require D3D11.1 or D3D12
        m_Supported = SystemInfo.graphicsDeviceVersion.Contains("level 11.1") || SystemInfo.graphicsDeviceVersion.Contains("level 12") || SystemInfo.graphicsDeviceType == GraphicsDeviceType.Vulkan;
#elif UNITY_PS4 || UNITY_XBOXONE
        // Always supported on main consoles.
        m_Supported = true;
#else
        // Not supported elsewhere (aka. how to check support on other platforms?)
        m_Supported = SystemInfo.graphicsDeviceType == UnityEngine.Rendering.GraphicsDeviceType.Metal;
#endif
        ...
        if(!m_Supported)
        {
            ...
            Debug.LogErrorFormat(
                "DepthOfFieldAutoFocus requires DirectX feature level 11.1 or higher (or equivalent for other platforms). Disabling feature.\n" +
                "Detected device type: {0}, version: {1}.",
