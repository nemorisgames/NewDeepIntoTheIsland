		...
		public void _UpdateFPCamera ()
		{
			...
			if (headBob)
			{
				if (headBobMethod == FirstPersonHeadBobMethod.BuiltIn)
				{
					...
					bobTimer += Mathf.Abs (player.GetMoveSpeed ()) * Time.deltaTime * 5f;
			...
			if (allowMouseWheelZooming && _camera != null && KickStarter.stateHandler.gameState == AC.GameState.Normal)
			{
				float scrollWheelInput = KickStarter.playerInput.InputGetAxis ("Mouse ScrollWheel");
				if (scrollWheelInput > 0f)
				{
					_camera.fieldOfView = Mathf.Max (_camera.fieldOfView - 3, minimumZoom);
				}
				else if (scrollWheelInput < 0f)
				{
					_camera.fieldOfView = Mathf.Min (_camera.fieldOfView + 3, maximumZoom);
				}
			}
		}
