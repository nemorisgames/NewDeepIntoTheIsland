...
namespace AC
{
	...
	public class MenuElement : ScriptableObject
	{
		...
		public virtual void ShowGUI (Menu menu)
		{
			...
			{
				singleSlotBackgrounds = CustomGUILayout.ToggleLeft ("Background texture is per slot?", singleSlotBackgrounds, apiPrefix + ".singleSlotBackgrounds");
			}
		...
		{
			if (backgroundTexture && _slot == 0 && !singleSlotBackgrounds)
			{
				GUI.DrawTexture (ZoomRect (relativeRect, zoom), backgroundTexture, ScaleMode.StretchToFill, true, 0f);
			}
			else if (backgroundTexture && singleSlotBackgrounds)
			{
