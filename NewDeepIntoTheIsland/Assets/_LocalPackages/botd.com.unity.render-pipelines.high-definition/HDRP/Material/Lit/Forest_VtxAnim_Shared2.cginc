...
void AnimateVegetationHierarchyPivot(inout float3 worldPos, inout float3 worldNrm, float3 pivotData, float3 pivotColor, float3 objectRoot, float timeNudge) {
	...
	float lvBRelativeObjectScale = mul(UNITY_MATRIX_M, float4(0, _WindRangeLvlB, 0, 0)).y;
...
#if 1
	if(pivotEnabled0) {
		float3 lv0Pos = mul(UNITY_MATRIX_M, float4(pivotPos0, 1)).xyz;
		float3 lv0PosStatic = lv0Pos;
		float3 lv0Fwd = normalize(mul((float3x3)UNITY_MATRIX_M, pivotFwd0));
		...
		float lv0DistScale = saturate(length(worldPos - lv0Pos) / mul(UNITY_MATRIX_M, float4(0, _WindRangeLvl0, 0, 0)).y);
...
#if 1
		if(pivotEnabled1) {
			float3 lv1Pos = mul(UNITY_MATRIX_M, float4(pivotPos1, 1)).xyz;
			float3 lv1PosStatic = lv1Pos;
			float3 lv1Fwd = normalize(mul((float3x3)UNITY_MATRIX_M, pivotFwd1));
			...
			float lv1DistScale = saturate(length(worldPos - lv1Pos) / mul(UNITY_MATRIX_M, float4(0, _WindRangeLvl1, 0, 0)).y);
...
void AnimateVegetationSinglePivot(inout float3 worldPos, inout float3 worldNrm, float3 pivotData, float3 pivotColor, float timeNudge) {
	...
	float3 worldElementPivot = mul(UNITY_MATRIX_M, float4(pivotPos, 1)).xyz;
	float3 worldElementPivotFwd = mul((float3x3)UNITY_MATRIX_M, pivotFwd);
...
void AnimateVegetationProceduralBranch(inout float3 worldPos, inout float3 worldNrm, float3 objectRoot, float timeNudge) {
	...
	float3 worldElementPivotFwd = length(mul((float3x3)UNITY_MATRIX_M, float3(1, 1, 1)) / 1.42f) * float3(0, 1, 0);
