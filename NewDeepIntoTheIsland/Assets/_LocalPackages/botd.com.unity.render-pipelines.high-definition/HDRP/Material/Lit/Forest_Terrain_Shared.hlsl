...
PackedVaryingsType TerrainSharedVert(TerrainVtxAttribs input) {
	...
	output.positionWS = TransformObjectToWorld(vertex);
	output.positionWS = GetCameraRelativePositionWS(output.positionWS);
...
SurfaceData GetSurfaceDataTerrain(FragInputs input, float3 V, inout PositionInputs posInput, out float weight, out float depthOffset, out float grassOcclusion) {
	...
	surfaceData.skyOcclusion = SampleSkyOcclusion(input.positionWS, input.color.xy, grassOcclusion);
	grassOcclusion = 1;
