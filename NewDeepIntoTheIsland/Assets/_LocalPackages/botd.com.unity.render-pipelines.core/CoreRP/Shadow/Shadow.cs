...
//forest-end:
        ...
        protected bool Layout()
        {
            ...
            for (uint i = 0; i < m_ActiveEntriesCount; ++i)
            {
                ...
                {
                    Debug.LogWarning("Shadow atlasing has failed curx:" + curx + " vp.width:" + vp.width + " xmax:" + xmax);
                    return false;
                }
                vp.x = curx;
                vp.y = cury;
    ...
    public class ShadowManager : ShadowManagerBase
    {
        ...
        protected override bool AllocateShadows(FrameId frameId, Camera camera, bool cameraRelativeRendering, List<VisibleLight> lights, uint totalGranted, ref ShadowRequestVector grantedRequests, ref ShadowIndicesVector shadowIndices, ref ShadowDataVector shadowDatas, ref ShadowPayloadVector shadowmapPayload)
        {
            ...
            for (uint i = 0, cnt = grantedRequests.Count(); i < cnt; ++i)
            {
                ...
                {
//forest-begin: Use GetShadowResolution rather than just accessing the member
                    if (m_ShadowmapsPerType[(int)shadowtype, smidx] != null && m_ShadowmapsPerType[(int)shadowtype, smidx].Reserve(frameId, camera, cameraRelativeRendering, ref sd, grantedRequests[i], (uint)AdditionalShadowData.GetShadowResolution(asd), (uint)AdditionalShadowData.GetShadowResolution(asd), ref shadowDatas, ref shadowmapPayload, lights))
                        break;
                    smidx++;
//forest-end:
                }
