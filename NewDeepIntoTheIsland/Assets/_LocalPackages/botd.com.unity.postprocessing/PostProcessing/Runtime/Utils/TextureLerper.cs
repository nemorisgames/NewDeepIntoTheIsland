...
namespace UnityEngine.Rendering.PostProcessing
{
    class TextureLerper
    {
        ...
        RenderTexture Get(RenderTextureFormat format, int w, int h, int d = 1, bool enableRandomWrite = false, bool force3D = false)
        {
            ...
            {
                var r = m_Recycled[i];
                if (r.width == w && r.height == h && r.volumeDepth == d && r.format == format && r.enableRandomWrite == enableRandomWrite && r.dimension == dimension)
                {
                    rt = r;
                    break;
                }
        ...
        internal Texture Lerp(Texture from, Texture to, float t)
        {
            ...
            if (is3D)
            {
                ...
                int size = from.width > from.height ? from.width : from.height;
                size = size > dpth ? size : dpth;
